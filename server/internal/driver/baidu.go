...
func NewBaidu(p *model.Policy) (fscore.Driver, error) {
	o := p.Opts()
	if o["access_token"] == "" {
		return nil, fmt.Errorf("百度网盘需要 access_token（开放平台 OAuth 授权后获取）")
	}
	return &Baidu{
...
// BaiduAuthURL 构造授权页地址
func BaiduAuthURL(clientID string) string {
	return "https://openapi.baidu.com/oauth/2.0/authorize?response_type=code&client_id=" + url.QueryEscape(clientID) + "&redirect_uri=oob&scope=basic,netdisk"
}

// BaiduExchangeCode 授权码换 token
func BaiduExchangeCode(clientID, clientSecret, code string) (access string, err error) {
	u := fmt.Sprintf("https://openapi.baidu.com/oauth/2.0/token?grant_type=authorization_code&code=%s&client_id=%s&client_secret=%s&redirect_uri=oob",
		url.QueryEscape(code), url.QueryEscape(clientID), url.QueryEscape(clientSecret))
	m, err := jreq("GET", u, nil, nil)
	if err != nil {
		return "", err
	}
	return jstr(m, "access_token"), nil
}
...
func (d *Baidu) List(dir string) ([]fscore.Entry, error) {
	...
	u := fmt.Sprintf("%s/file?method=list&access_token=%s&dir=%s&limit=1000", apiBaidu, d.AccessToken, url.QueryEscape(c))
	m, err := jreq("GET", u, nil, nil)
	if err != nil {
...
func (d *Baidu) Mkdir(dir string) error {
	...
	_, err = jreq("POST", apiBaidu+"/file?method=create&access_token="+d.AccessToken,
...
func (d *Baidu) Rename(p, newName string) error {
	...
	_, err := jreq("POST", apiBaidu+"/filemanager?method=filemanager&opera=rename&access_token="+d.AccessToken,
		map[string]string{"Content-Type": "application/x-www-form-urlencoded"}, []byte(form.Encode()))
	d.invalidate(c)
	_ = newPath
	return wrapBaiduErr(err)
...
func (d *Baidu) Move(src, dstDir string) error {
	...
	_, err := jreq("POST", apiBaidu+"/filemanager?method=filemanager&opera=move&access_token="+d.AccessToken,
		map[string]string{"Content-Type": "application/x-www-form-urlencoded"}, []byte(form.Encode()))
	d.invalidate(sc)
	return wrapBaiduErr(err)
}
...
func (d *Baidu) Copy(src, dstDir string) error {
	...
	dc, _ := fscore.Clean(dstDir)
	form := url.Values{"filelist": {fmt.Sprintf(`[{"path":%q,"dest":%q,"newname":%q}]`, sc, dc, path.Base(sc))}}
	_, err := jreq("POST", apiBaidu+"/filemanager?method=filemanager&opera=copy&access_token="+d.AccessToken,
		map[string]string{"Content-Type": "application/x-www-form-urlencoded"}, []byte(form.Encode()))
	return wrapBaiduErr(err)
}
...
func (d *Baidu) Delete(p string) error {
	...
	_, err := jreq("POST", apiBaidu+"/filemanager?method=filemanager&opera=delete&access_token="+d.AccessToken,
		map[string]string{"Content-Type": "application/x-www-form-urlencoded"}, []byte(form.Encode()))
	d.invalidate(c)
	return wrapBaiduErr(err)
}
...
func (d *Baidu) DirectURL(p string) (string, error) {
	...
	m, err := jreq("POST", fmt.Sprintf("%s/multimedia?method=filemetas&access_token=%s&dlink=1", apiBaidu, d.AccessToken),
		map[string]string{"Content-Type": "application/x-www-form-urlencoded"}, []byte(form.Encode()))
	if err != nil {
		return "", err
	}
	...
	return jstr(list[0].(map[string]interface{}), "dlink") + "&access_token=" + d.AccessToken, nil
}
...
func (d *Baidu) Quota() (used, total int64, err error) {
	m, err := jreq("GET", fmt.Sprintf("%s/index?method=quota&access_token=%s", apiBaidu, d.AccessToken), nil, nil)
	if err != nil {
		return 0, 0, err
	}
