...
func (h *AdminHandler) GroupSave(c *gin.Context) {
	...
	if err := model.DB.Model(&model.UserGroup{}).Where("id = ?", in.ID).Updates(map[string]interface{}{
		...
		// 真实错误进日志（远端 DB 抖动/唯一索引冲突等），接口仍返回通用文案
		log.Printf("[CloudPan] 保存用户组 %s(id=%d) 失败: %v", in.Name, in.ID, err)
		dto.Fail(c, 400, "保存失败")
		return
	}
...
type policyIn struct {
	...
	// QuotaGB 分区标称大小（GB）：0=自动（本地盘取根目录所在分区真实大小）
	...
	for _, p := range items {
		...
		if p.Type == "local" {
			if p.QuotaBytes > 0 {
				...
				total = fscore.StatfsTotalBytes(p.RootPath)
			}
		}
		out = append(out, gin.H{
			"id": p.ID, "name": p.Name, "letter": p.Letter, "type": p.Type,
			"rootPath": p.RootPath, "options": p.Opts(), "status": p.Status,
			"statusMsg": p.StatusMsg, "usageBytes": p.UsageBytes,
			"quotaBytes": p.QuotaBytes, "totalBytes": total, "createdAt": p.CreatedAt,
		})
	}
	dto.OK(c, out)
}
...
func (h *AdminHandler) PolicyCreate(c *gin.Context) {
	...
	p := model.Policy{Name: in.Name, Letter: in.Letter, Type: in.Type, RootPath: in.RootPath, Options: opts, CreatedBy: u.ID, QuotaBytes: quotaBytesFromGB(in.QuotaGB)}
	if in.Type == "local" {
		if in.RootPath == "" {
...
func (h *AdminHandler) PolicyUpdate(c *gin.Context) {
	...
	model.DB.Model(&p).Updates(map[string]interface{}{
		...
		"quota_bytes": quotaBytesFromGB(in.QuotaGB),
	})
	h.Site.Fs.Invalidate(p.ID)
	_ = model.DB.First(&p, c.Param("id")).Error
	dto.OK(c, p)
}
...
// UserShareAudit 站内用户共享审计。
// 与外链「分享审计」区分：站内共享把目录直接授权给目标（用户/组/所有人），无链接/提取码，
// 目标方登录后即在「共享」中随时可见可下载——这里列出全部授权关系 + 访问计数，供管理员排查与回收。
func (h *AdminHandler) UserShareAudit(c *gin.Context) {
	var in dto.PageIn
	_ = c.ShouldBindQuery(&in)
	offset, limit := in.Normalize()
	q := model.DB.Model(&model.UserShare{})
	if in.Keyword != "" {
		kw := "%" + in.Keyword + "%"
		q = q.Where("name LIKE ? OR path LIKE ?", kw, kw)
	}
	var total int64
	q.Count(&total)
	...
	// 批量取 属主 / 目标(用户|组) / 策略名，避免 N+1
	ownerIDs, tUserIDs, tGroupIDs, pIDs := []uint{}, []uint{}, []uint{}, []uint{}
	for _, s := range items {
		ownerIDs = append(ownerIDs, s.OwnerID)
		pIDs = append(pIDs, s.PolicyID)
		tt := s.TargetType
		if tt == "" {
			tt = "user"
		}
		if tt == "user" {
			tUserIDs = append(tUserIDs, s.TargetID)
		} else if tt == "group" {
			tGroupIDs = append(tGroupIDs, s.TargetID)
		}
	}
	owners := map[uint]model.User{}
	if len(ownerIDs) > 0 {
		var us []model.User
		model.DB.Where("id IN ?", ownerIDs).Find(&us)
		for _, u := range us {
			owners[u.ID] = u
		}
	}
	tUsers := map[uint]string{}
	if len(tUserIDs) > 0 {
		var us []model.User
		model.DB.Where("id IN ?", tUserIDs).Find(&us)
		for _, u := range us {
			tUsers[u.ID] = u.Username
		}
	}
	tGroups := map[uint]string{}
	if len(tGroupIDs) > 0 {
		var gs []model.UserGroup
		model.DB.Where("id IN ?", tGroupIDs).Find(&gs)
		for _, g := range gs {
			tGroups[g.ID] = g.Name
		}
	}
	polNames := map[uint]string{}
	if len(pIDs) > 0 {
		var ps []model.Policy
		model.DB.Where("id IN ?", pIDs).Find(&ps)
		for _, p := range ps {
			polNames[p.ID] = p.Name
			if p.Letter != "" {
				polNames[p.ID] += " (" + p.Letter + ":)"
			}
		}
	}
	out := make([]gin.H, 0, len(items))
	for _, s := range items {
		...
		targetName := "所有人"
		if tt == "user" {
			targetName = tUsers[s.TargetID]
			if targetName == "" {
				targetName = "#" + fmt.Sprint(s.TargetID)
			}
		} else if tt == "group" {
			targetName = tGroups[s.TargetID]
			if targetName == "" {
				targetName = "组#" + fmt.Sprint(s.TargetID)
			}
		}
		o := owners[s.OwnerID]
		out = append(out, gin.H{
			"id": s.ID, "name": s.Name, "path": s.Path, "perm": s.Perm,
			"policy": polNames[s.PolicyID],
			"owner": o.Nickname, "ownerName": o.Username,
...
// UserShareCancel 管理员取消站内共享（与分享审计的取消分享对应）
func (h *AdminHandler) UserShareCancel(c *gin.Context) {
	model.DB.Delete(&model.UserShare{}, c.Param("id"))
	middleware.Audit(c, "admin", "管理员取消站内共享 #" + c.Param("id"))
	dto.OK(c, nil)
}
...
// quotaBytesFromGB 分区标称大小（GB → 字节）：<=0 → 0（自动，本地盘取真实分区大小）；>0 封顶 1PB
func quotaBytesFromGB(gb int64) int64 {
	if gb <= 0 {
		return 0
	}
	const maxGB = 1024 * 1024 // 1PB
	if gb > maxGB {
		gb = maxGB
	}
	return gb << 30
