...
func (h *SiteHandler) Policies(c *gin.Context) {
	...
	for _, p := range list {
		// 内置资源库（T:）不进入非管理员的资源管理器盘列表：
		// 它是海报应用的素材来源，应用内经 /api/poster/builtin/policy 专属接口取策略，
		// 不暴露为普通用户可浏览的盘
		if x.user.Role != "admin" && p.Type == "builtin" {
			continue
		}
		...
		if p.Type == "local" && (p.UsageAt == nil || time.Since(*p.UsageAt) > 10*time.Minute) {
			// 按策略根（全部用户的隔离目录）汇总整盘用量。
			// 不能按当前查看者的隔离目录算：那会把"某一个人的用量"写进策略级字段（last-viewer-wins），
			// 多人使用同一盘时显示值随最后访问者漂移。
			if d, err := h.Fs.DriverFor(&p, nil); err == nil {
		...
		// 显示口径按角色分离（网盘惯例，用户权限分离）：
		// 管理员 = 「整盘已用 / 分区大小」——分区大小 = 挂载时设定的虚拟大小，未设定则取
		//   根目录所在分区的真实大小（statfs）；已用 = 全部用户隔离目录之和。
		// 非管理员 = 「本人已用 / 本人配额」——配额 = 个人覆盖 > 用户组配额（0=不限量）；
		//   不向普通用户/访客暴露整盘容量与总用量（那是分区级信息）。
		if x.user.Role == "admin" {
			if p.Type == "local" {
				if p.QuotaBytes > 0 {
					p.TotalBytes = p.QuotaBytes
				} else {
					p.TotalBytes = fscore.StatfsTotalBytes(p.RootPath)
				}
			}
		} else {
			p.UsageBytes = x.user.UsedBytes
			if q, limited := effectiveQuotaBytes(x.user, x.group); limited {
				p.TotalBytes = q
			}
		}
...
// touchPolicyUsage 清除指定本地策略的用量缓存（usage_at=NULL）：
// 文件变更（删除/恢复/上传/复制/压缩解压等）后用量立即失效，下次 policies 请求即重算，
// 不必等 10 分钟懒重算窗口——否则"删了 790MB 文件，盘上仍显示占用 790MB"最长持续 10 分钟。
func touchPolicyUsage(ids ...uint) {
	if len(ids) == 0 {
		return
	}
	model.DB.Model(&model.Policy{}).
		Where("id IN ? AND type = ?", ids, "local").
		Updates(map[string]interface{}{"usage_at": nil})
}
...
func (h *SiteHandler) Rename(c *gin.Context) {
	...
	// 版本历史跟随文件迁移（同目录重命名：新旧路径路由到同一物理根）
	if ld := fscore.LocalRoot(d, in.Path); ld != nil {
...
func (h *SiteHandler) Move(c *gin.Context) {
	...
	for _, src := range in.Paths {
		...
		// 版本相关操作需按路径路由到具体物理根；管理员视图下源/目标可能跨根，
		// 跨根移动不迁移版本（版本文件无法跨根物理移动，保持历史行为）
		srcRoot := fscore.LocalRoot(d, srcClean)
		dstRoot := fscore.LocalRoot(d, dst)
		// 原地移动 = 无操作
		if parentOf(src) == dst {
			continue
		}
		...
		if in.Overwrite {
			if tgtVP, jerr := fscore.Join(dst, baseOf(srcClean)); jerr == nil {
				if te, terr := d.Stat(tgtVP); terr == nil {
					...
					if dstRoot != nil {
						if tp, perr := dstRoot.Physical(tgtVP); perr == nil {
		...
		// 版本历史跟随文件迁移（同物理根时；跨根移动版本无法物理跟随，保持历史行为跳过）
		if srcRoot != nil && srcRoot == dstRoot {
			if newVP, err := fscore.Join(dst, baseOf(srcClean)); err == nil {
				fscore.MigrateVersions(srcRoot, p.ID, srcClean, newVP, isDir)
			}
		}
	}
...
func addQuota(userID uint, delta int64) {
	...
	model.DB.Model(&model.User{}).Where("id = ?", userID).
		UpdateColumn("used_bytes", gorm.Expr("GREATEST(used_bytes + ?, 0)", delta))
}

// quotaAccountOwner 上传落盘文件的配额属主：本地盘按物理路径一级目录定位
// （管理员经管理视图写入他人目录时计入属主配额）；无法定位/非本地盘返回 nil=按操作者计。
func quotaAccountOwner(x ctx3, p *model.Policy, d fscore.Driver, vp string) *model.User {
	if p == nil || p.Type != "local" || d == nil {
		return nil
	}
	phys, err := fscore.PhysicalOf(d, vp)
	if err != nil {
		return nil
	}
	return fscore.OwnerOfPhys(p.RootPath, phys)
}

// commitQuotaCore 上传净增量的原子配额提交（借鉴 Cloudreve 的条件 UPDATE 原子配额）：
// 正增量用单条 SQL 原子校验「used + delta <= limit」——并发上传时后到者直接失败，
// 消除两个请求各自读请求头 UsedBytes 再写绝对值造成的超配额与丢失更新窗口；
// 非正增量（小文件覆盖大文件）直接走 addQuota 扣减。
// owner=配额记账对象（nil=操作者本人，见 quotaAccountOwner）。
// 返回 (是否成功, 失败文案)。不写 HTTP 响应——同步接口用包装 commitQuotaUpload，
// 异步合并（无请求上下文）直接调本函数。超限返回 false，调用方须回滚落盘文件。
func commitQuotaCore(x ctx3, owner *model.User, delta int64) (bool, string) {
	if owner == nil {
		owner = x.user
	}
	if delta <= 0 {
		addQuota(owner.ID, delta)
		return true, ""
	}
	var u model.User
	if model.DB.Select("id", "quota_mb", "group_id", "used_bytes").First(&u, owner.ID).Error != nil {
		addQuota(owner.ID, delta)
		return true, ""
	}
	group := x.group
	if owner.ID != x.user.ID && u.GroupID > 0 {
		// 属主配额按属主所在组
		var og model.UserGroup
		group = nil
		if model.DB.First(&og, u.GroupID).Error == nil {
			group = &og
		}
	}
	limit, limited := effectiveQuotaBytes(&u, group)
	if !limited {
		addQuota(owner.ID, delta)
		return true, ""
	}
	res := model.DB.Model(&model.User{}).
		Where("id = ? AND used_bytes + ? <= ?", owner.ID, delta, limit).
		UpdateColumn("used_bytes", gorm.Expr("used_bytes + ?", delta))
	if res.Error != nil || res.RowsAffected == 0 {
		NotifyQuotaExceeded(owner.ID, limit>>20, u.UsedBytes)
		return false, fmt.Sprintf("超出配额：已用 %dMB / 上限 %dMB", u.UsedBytes>>20, limit>>20)
	}
	return true, ""
}

// commitQuotaUpload 同步上传接口的配额提交包装：超限写 403 响应并返回 false
func commitQuotaUpload(c *gin.Context, x ctx3, owner *model.User, delta int64) bool {
	ok, msg := commitQuotaCore(x, owner, delta)
	if !ok {
		dto.Fail(c, 403, msg)
		return false
	}
...
func (h *SiteHandler) Copy(c *gin.Context) {
	...
	touchPolicyUsage(p.ID) // 复制产生新副本，用量增加
...
func (h *SiteHandler) removeSource(d fscore.Driver, p *model.Policy, src string, x ctx3) error {
	if phys, err := fscore.PhysicalOf(d, src); err == nil {
		...
		model.DB.Create(&model.RecycleItem{UserID: x.user.ID, OwnerID: recycleOwnerID(p, phys), PolicyID: p.ID, OrigPath: parentOf(src),
...
// recycleOwnerID 本地盘按物理路径一级目录定位文件属主（非本地/无法定位 = 0，退款时回退删除者）
func recycleOwnerID(p *model.Policy, phys string) uint {
	if p.Type != "local" {
		return 0
	}
	if ow := fscore.OwnerOfPhys(p.RootPath, phys); ow != nil {
		return ow.ID
	}
	return 0
}

// recycleRefundID 回收站项配额退款对象：优先属主，历史记录（OwnerID=0）回退删除者
func recycleRefundID(item *model.RecycleItem) uint {
	if item.OwnerID > 0 {
		return item.OwnerID
	}
	return item.UserID
...
func (h *SiteHandler) CrossCopy(c *gin.Context) {
	...
	addQuota(x.user.ID, add)
	touchPolicyUsage(dp.ID) // 目标盘产生新副本（源保留，仅目标盘用量变化）
...
func (h *SiteHandler) CrossMove(c *gin.Context) {
	...
	moveIDs := []uint{dp.ID}
	for _, s := range srcs {
		moveIDs = append(moveIDs, s.sp.ID)
	}
	touchPolicyUsage(moveIDs...) // 目标盘增加、本地源盘进回收站减少
...
func (h *SiteHandler) Delete(c *gin.Context) {
	...
	for _, src := range in.Paths {
		...
		model.DB.Create(&model.RecycleItem{UserID: x.user.ID, OwnerID: recycleOwnerID(p, physSrc), PolicyID: p.ID, OrigPath: parentOf(src),
			TrashPath: trashRel, Name: e.Name, IsDir: e.IsDir, Size: size, DeletedAt: time.Now()})
	}
	touchPolicyUsage(p.ID) // 文件已移出盘根（进回收站），用量立即失效
...
func (h *SiteHandler) WriteText(c *gin.Context) {
	...
	if add != 0 {
		model.DB.Model(&model.User{}).Where("id = ?", x.user.ID).
			UpdateColumn("used_bytes", gorm.Expr("GREATEST(used_bytes + ?, 0)", add))
	}
	if p.Type == "local" {
		if phys, err := fscore.PhysicalOf(d, vp); err == nil {
	...
	touchPolicyUsage(p.ID) // 文本保存改变文件大小
...
func (h *SiteHandler) RecycleRestore(c *gin.Context) {
	...
	touchPolicyUsage(restoredIDs...) // 恢复使文件回到盘根，用量增加
	dto.OK(c, nil)
}
