...
func (d *LocalDriver) Quota() (used, total int64, err error) {
	...
	// total = 根目录所在分区的真实总容量（statfs）；此前恒为 0，前端只能瞎猜
	total = StatfsTotalBytes(d.Root)
...
// OwnerOfPhys 把本地策略根下的物理路径映射回属主用户（一级目录即用户隔离目录）。
// 配额记账/回收站退款按属主而非删除者：管理员代删访客文件时，退款必须回到访客头上。
// 无法映射（不在用户目录内、目录已改名失联等）返回 nil。
func OwnerOfPhys(rootPath, phys string) *model.User {
	rel, err := filepath.Rel(rootPath, phys)
	...
	var u model.User
	// 纯 ASCII 用户目录名即用户名（UserDirOf 的命名规则），优先按用户名查
	if err := model.DB.Where("username = ?", seg).First(&u).Error; err == nil {
		return &u
	}
	// user_<id> / 中文用户名等固定目录名记录在 UserSetting(local_dir)
	var s model.UserSetting
	if err := model.DB.Where("`key` = ? AND value = ?", "local_dir", seg).First(&s).Error; err == nil {
		if model.DB.First(&u, s.UserID).Error == nil {
			return &u
		}
	}
	return nil
