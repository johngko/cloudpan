...
// SaveChunk 写入单个分片并记账。返回当前已收分片数。
// 记账（received）读/写失败时返回错误——分片数据本身已落盘，
// 客户端重试同一分片是安全的（os.Create 覆盖写），重试后记账重新执行。
func (s *Service) SaveChunk(sid string, idx int, r io.Reader) (int, error) {
	var sess model.UploadSession
	if err := model.DB.First(&sess, "id = ?", sid).Error; err != nil || sess.Status != "uploading" {
		return 0, errors.New("上传会话不存在或已结束")
	}
	if idx < 0 || idx >= sess.TotalChunks {
		return 0, errors.New("分片序号非法")
	}
	_ = os.MkdirAll(filepath.Join(s.TmpDir, sid), 0o755)
	...
	if n > sess.ChunkSize+1024 {
		...
		return 0, errors.New("分片过大")
	}
	s.mu.Lock()
	// 锁内重新读取最新 Received，避免并发上传同会话多个分片时基于过期值互相覆盖、丢失已上传分片记录
	var fresh model.UploadSession
	if err := model.DB.Select("received").First(&fresh, "id = ?", sid).Error; err != nil {
		s.mu.Unlock()
		return 0, fmt.Errorf("读取上传会话记录失败: %w", err)
	}
	...
	if err := model.DB.Model(&model.UploadSession{}).Where("id = ?", sid).
		Updates(map[string]interface{}{"received": string(rc), "updated_at": time.Now()}).Error; err != nil {
		s.mu.Unlock()
		return 0, fmt.Errorf("更新上传进度失败: %w", err)
	}
	s.mu.Unlock()
...
func (s *Service) AbortUpload(sid string, uid uint) error {
	...
	_ = os.RemoveAll(filepath.Join(s.TmpDir, sess.ID))
	// 顺带清理中断合并可能残留的 .assembling（尽力而为，不阻塞取消）
	s.removeAssembling(&sess, uid)
	return nil
}

// removeAssembling 删除会话目标路径下残留的合并临时文件
func (s *Service) removeAssembling(sess *model.UploadSession, uid uint) {
	var u model.User
	...
	d, err := s.DriverFor(&p, &u)
	if err != nil {
		return
	}
	vp, err := Join(sess.ParentPath, sess.Name)
	if err != nil {
		return
	}
	phys, err := PhysicalOf(d, vp)
	if err != nil {
		return
	}
	_ = os.Remove(phys + ".assembling")
}
...
// 管理员本地盘视图（AdminLocalDriver）按其路由规则落到具体物理根，
// 否则管理员在用户目录内的删除/移动/恢复全部会误报"非本地存储"。
func PhysicalOf(d Driver, vp string) (string, error) {
	switch ld := d.(type) {
	case *LocalDriver:
		return ld.Physical(vp)
	case *AdminLocalDriver:
		return ld.route(vp).Physical(vp)
	default:
		return "", errors.New("非本地存储")
	}
}

// LocalRoot 取本地存储按 vp 路由到的底层本地驱动（版本迁移/归档等需要具体物理根）。
// 非本地存储返回 nil；管理员视图下不同路径可能路由到不同物理根，跨根操作由调用方判等。
func LocalRoot(d Driver, vp string) *LocalDriver {
	switch ld := d.(type) {
	case *LocalDriver:
		return ld
	case *AdminLocalDriver:
		return ld.RouteFor(vp)
	default:
		return nil
	}
}
