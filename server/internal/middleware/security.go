...
func buildCSP(origins []string) string {
	...
	csp := "default-src 'self'; " +
		...
		// data:：TOAST UI 图片编辑器等组件以 data:text/css 动态注入图标主题样式；
		// data: 内容无法发起网络请求，不扩大外部可达面（img/font 已放行 data:）
		"style-src 'self' 'unsafe-inline' data:; " +
		"img-src 'self' data: blob:; " +
		"media-src 'self' blob:; " +
		// api.open-meteo.com / air-quality-api.open-meteo.com：小组件天气卡数据源
		// （免费公共天气/空气质量 API，只读 GET，无 key），按主机精确放行；
		// 离线时前端优雅降级显示「暂无法获取天气」/隐藏 AQI 徽章
		// blob:：/webapps/* 移植应用（如音乐播放器）用页面内创建的 Blob URL 喂给
		// 内嵌播放器的 fetch；blob URL 只能由同源页面创建，不扩大外部可达面
		"connect-src 'self' blob: https://api.open-meteo.com https://air-quality-api.open-meteo.com; " +
		"font-src 'self' data:; " +
		"frame-src 'self'; " +
...
func SecurityHeaders() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// 多功能编辑器（ace，v9os 商店移植）的 Emmet 缩写引擎用 eval/new Function
		// 展开用户输入的缩写（如 div.class 制表补全）；Ace 的语言 worker 在 basePath
		// 解析失败时回退到 Blob URL 建 worker。仅该 webapp 的 iframe 文档受影响，
		// 按路径级例外放宽（worker 源码来自同源 src-min，不扩大外部面）。
		if strings.HasPrefix(c.Request.URL.Path, "/webapps/ace/") {
			csp = strings.Replace(csp, "script-src 'self' 'unsafe-inline'", "script-src 'self' 'unsafe-inline' 'unsafe-eval'", 1)
			csp = csp + "; worker-src 'self' blob:"
		}
		// 在线PS（photopea，v9os 商店移植）：pp.js 内有 23 处 eval（字体/JS 解析），
		// 无 'unsafe-eval' 应用无法正常启动；其字体 CDN（st0.dancf.com）维持 connect-src
		// 阻断（缺字体时渲染降级，不为此放行外部面）。仅该 webapp 子树放宽。
		if strings.HasPrefix(c.Request.URL.Path, "/webapps/photopea/") {
			csp = strings.Replace(csp, "script-src 'self' 'unsafe-inline'", "script-src 'self' 'unsafe-inline' 'unsafe-eval'", 1)
		}
		// CAD 看图（webapps/cad，mlightcad cad-simple-viewer）：LibreDWG DWG 解析 worker
		// 需要 WebAssembly.instantiate（'wasm-unsafe-eval'）且其 emscripten 运行时会
		// eval 胶水字符串（需 'unsafe-eval'）。仅该 webapp 子树放宽（wasm 为同源
		// 第一方资源，不扩大外部面）。
		if strings.HasPrefix(c.Request.URL.Path, "/webapps/cad/") {
			csp = strings.Replace(csp, "script-src 'self' 'unsafe-inline'", "script-src 'self' 'unsafe-inline' 'unsafe-eval'", 1)
		}
...
// v9os 商店移植 webapps（/webapps/<code>/，自包含第一方静态应用）。
