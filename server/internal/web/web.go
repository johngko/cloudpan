...
// setCacheHeaders 静态资源缓存策略：
//   - /assets/*：Vite 内容哈希文件名，构建即换名，可一年 immutable（老 hash 永不再变，
//     升级后 index.html 无缓存会指向新 hash，浏览器自然拿新文件）；
//   - 其余静态目录（fonts/vendor/icons/wallpapers/excalidraw-assets/webapps）：文件名稳定不带
//     哈希，给 1 天 max-age + 继续走 Last-Modified 协商，兼顾速度与新内容可达性。
func setCacheHeaders(c *gin.Context, name string) {
	...
	switch {
	case strings.HasPrefix(name, "fonts/"), strings.HasPrefix(name, "vendor/"),
		strings.HasPrefix(name, "icons/"), strings.HasPrefix(name, "wallpapers/"),
		strings.HasPrefix(name, "excalidraw-assets/"), strings.HasPrefix(name, "webapps/"):
