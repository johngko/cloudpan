...
func fsService(cfg *config.Config) *fscore.Service {
	...
	fscore.RegisterDriver("local", func(p *model.Policy, u *model.User) (fscore.Driver, error) {
		// 管理员本地盘视图：本人文件仍在根，其他用户以「用户文件夹」形式暴露，
		// 管理员可进入任意用户目录查看/管理其在该盘存放的文件
		if u != nil && u.Role == "admin" {
			return fscore.NewAdminLocalView(p.RootPath, fscore.UserDirOf(u))
		}
		root := p.RootPath
...
func main() {
	...
	// 读侧超时：卡住不传 body 的分片连接会无限占用 goroutine/FD/已创建的分片文件
	// （慢速上传 4MB 分片远小于 30min，正常流量不受影响）。
	// 不设 WriteTimeout：SSE/通知长连接会被误杀。
	if err := (&http.Server{
		Handler:           r,
