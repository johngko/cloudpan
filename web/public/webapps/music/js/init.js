(function(){
  ...
  var rootPath = rootFrom(script.src, 4);
  ...
  window.smartInitHook = function(config){
    config.versionUrl = '';
    config.plugins.music = [];
    var webos = null;
    try{ webos = parent.webos; }catch(e){}
    if(!webos){ config.plugins.music.push({js: rootPath + '/common/sdk/plugins.sdk.js'}); }
    else{ window.webos = parent.webos; }
    config.plugins.music.push({js: rootPath + '/js/index-init.js'});
