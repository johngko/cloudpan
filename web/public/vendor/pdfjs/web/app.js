...
const PDFViewerApplication = {
  ...
  async initialize(appConfig) {
    ...
    // (vendored) Resolve the UI locale before the l10n instance is created:
    // an explicit ?locale= parameter wins, otherwise use the browser language.
    if (typeof PDFJSDev === "undefined" || PDFJSDev.test("GENERIC")) {
      const localeParams = new URLSearchParams(window.location.search);
      AppOptions.set("localeProperties", {
        lang: localeParams.get("locale") || navigator.language
      });
    }
