import { api, get, post, put, del, getToken, clearToken } from './http'
...
export interface Policy {
  ...
  // 展示口径按角色分离（后端计算）：
  // 管理员 = 整盘已用 / 分区大小（挂载时设定或真实分区大小）；非管理员 = 本人已用 / 本人配额
  totalBytes?: number // 0/缺省 = 不限量或未知（进度条中性显示）
  quotaBytes?: number // 管理员设定的分区标称大小（0=自动）
}
...
export const adminApi = {
  ...
  userShares: (page = 1, size = 100, keyword = '') => get<any>(`/admin/usershares?page=${page}&size=${size}&keyword=${encodeURIComponent(keyword)}`),
  userShareDelete: (id: number) => del(`/admin/usershares/${id}`),
...
export const uploadApi = {  init: (d: { policyId: number; parent: string; name: string; size: number; chunkSize: number; hash: string }) =>
  ...
  complete: (sessionId: string) =>
    // 服务端异步合并：complete 只做状态跃迁立即返回，但远端 DB 慢时跃迁链路本身也可能超过 60s
    post<any>('/upload/complete', { sessionId }, { timeout: 180000 }),
