  ...
  <div class="app-root">
    <div style="display: flex; flex: 1; overflow: hidden">
      ...
      <div class="ac-content">
                ...
                <div class="sys-block">
                  ...
                  <div v-for="d in sys.disks" :key="d.mount" class="sys-disk">
                    <div class="sys-disk-head">
                      <span class="sys-disk-name" :title="d.mount">{{ d.mount }}{{ d.label ? ' · ' + d.label : '' }}</span>
        ...
        <template v-else-if="tab === 'groups'">
          ...
          <div class="ac-cards" style="grid-template-columns: repeat(auto-fill, minmax(300px, 1fr))">
            <div v-for="g in groups" :key="g.id" class="ac-card" style="padding: 16px 18px">
              ...
              <div class="ac-kv" title="显式设置的应用权限（默认=继承允许；详情点「编辑」查看三态网格）"><span>应用权限</span><b>{{ permSummary(g) }}</b></div>
            ...
            <table class="ac-table">
              ...
              <tbody>
                <tr v-for="p in policies" :key="p.id">
                  ...
                  <td>
                    <div>{{ fmt(p.usageBytes) }}<span v-if="p.totalBytes > 0" style="color: var(--text-3)"> / {{ fmt(p.totalBytes) }}</span></div>
                    <div style="font-size: 11px; color: var(--text-3)">{{ p.totalBytes > 0 ? '剩余 ' + fmt(Math.max(0, p.totalBytes - p.usageBytes)) : (p.type === 'local' ? '未设分区大小（按真实分区）' : '云盘配额') }}</div>
                  </td>
                  <td>
        ...
        <!-- 共享审计（站内用户共享：目录直接授权给目标用户/组/所有人，无链接） -->
        <template v-else-if="tab === 'usershares'">
          <div class="ac-head">
            <div style="min-width: 0">
              <h2 class="ac-h2">共享审计</h2>
              <div style="font-size: 12px; color: var(--text-3); margin-top: 4px">与「分享审计」（外链）不同：站内共享把目录直接授权给目标，对方登录后即可在「共享」中随时查看/下载，无链接与提取码。</div>
            </div>
            <div style="display: flex; gap: 8px; flex: none">
              <input class="input" v-model="userShareKw" @keyup.enter="loadUserShares" placeholder="搜索目录名/路径" style="width: 200px" />
              <button class="btn" @click="loadUserShares">查询</button>
            </div>
          </div>
          <div class="ac-card" style="padding: 4px 0">
            <table class="ac-table">
              <thead><tr><th>目录</th><th>所在盘</th><th>共享者</th><th>共享给</th><th>权限</th><th>浏览/下载</th><th>创建时间</th><th style="width: 90px">操作</th></tr></thead>
              <tbody>
                <tr v-for="s in allUserShares" :key="s.id">
                  <td class="ac-strong" style="max-width: 200px; overflow: hidden; text-overflow: ellipsis" :title="s.path">{{ s.name }}<span style="color: var(--text-3); font-weight: 400; font-size: 11px">{{ s.path === '/' ? '（整盘）' : ' ' + s.path }}</span></td>
                  <td>{{ s.policy }}</td>
                  <td>{{ s.owner }} ({{ s.ownerName }})</td>
                  <td>{{ s.targetName }}<span style="color: var(--text-3); font-size: 11px">（{{ s.targetType === 'all' ? '所有人' : s.targetType === 'group' ? '用户组' : '用户' }}）</span></td>
                  <td>{{ s.perm === 'rw' ? '可写' : '只读' }}</td>
                  <td>{{ s.views }} / {{ s.downloads }}</td>
                  <td>{{ new Date(s.createdAt).toLocaleString() }}</td>
                  <td><button class="btn danger" style="padding: 4px 10px" @click="adminDelUserShare(s)">取消</button></td>
                </tr>
              </tbody>
            </table>
            <div v-if="!allUserShares.length" style="padding: 24px; text-align: center; color: var(--text-3); font-size: 13px">暂无站内共享</div>
          </div>
        </template>
    ...
    <div class="dialog-mask" v-if="permShow" @click.self="permShow = false">
      <div class="dialog" style="width: 560px">
        ...
        <div class="actions">
          <button class="btn" @click="permShow = false" :disabled="savingPerms">取消</button>
          <button class="btn primary" @click="savePerms" :disabled="savingPerms">{{ savingPerms ? '保存中…' : '保存' }}</button>
        </div>
      </div>
    </div>
    ...
    <div class="dialog-mask" v-if="groupShow" @click.self="groupShow = false">
      <div class="dialog" style="width: 560px">
        ...
        <div class="actions">
          <button class="btn" @click="groupShow = false" :disabled="savingGroup">取消</button>
          <button class="btn primary" @click="saveGroup" :disabled="savingGroup">{{ savingGroup ? '保存中…' : '保存' }}</button>
        </div>
      </div>
    </div>
    ...
    <div class="dialog-mask" v-if="policyShow" @click.self="closePolicy">
      <div class="dialog" style="width: 480px">
        ...
        <div class="row" v-if="editPolicy.type === 'local'">
          <label>分区大小（GB）</label>
          <input class="input" v-model.number="editPolicy.quotaGB" type="number" min="0" step="1"
            placeholder="留空 = 自动（根目录所在分区的真实大小）" style="width: 100%" />
          <div class="ac-guide" style="margin-top: 4px">管理员视角的分区总容量：留空取磁盘真实大小；填写则按此标称值显示「已用 / 总 / 剩余」（如整盘 5TB 中划 500GB 给此盘）</div>
        </div>
...
const tabs = computed(() => [
  ...
  { id: 'usershares', name: '共享审计', icon: 'share2' },
...
// 站内用户共享审计（与外链分享审计并列）
const allUserShares = ref<any[]>([])
const userShareKw = ref('')
const tasks = ref<any[]>([])
const logs = ref<any[]>([])
const logPage = ref(1)
...
async function savePerms() {
  ...
  if (savingPerms.value) return
  savingPerms.value = true
  try { await adminApi.userUpdate(u.id, { appPerms: permForm.value }); permShow.value = false; toast.success('权限已更新'); await loadAll() } catch (e: any) { toast.error(e.message) } finally { savingPerms.value = false }
}
...
const emptyPolicy = { type: 'local', name: '', letter: '', rootPath: '', quotaGB: 0 }
...
// 单飞合并：保存/操作后多处都会触发 loadAll（9 路并发 GET），
// 远端 DB 慢时连点保存会打出多倍重复请求、界面数据反复回跳——同一时刻只允许一轮
...
async function loadAll() {
  ...
  loadAllInflight = (async () => {
    try {
      ...
      allUserShares.value = (await adminApi.userShares(1, 200)).items
...
function taskTypeName(t: string) { return ({ offline: 'HTTP 离线', bt: 'BT/磁力链', compress: '压缩', decompress: '解压', upload: '上传', transfer: '转存' } as any)[t] || t }
...
async function saveGroup() {
  if (savingGroup.value) return // 保存中防重：远端 DB 慢响应时连点会打出多个重复写请求
  savingGroup.value = true
  try {
    await adminApi.groupSave({ ...editGroup.value, allowedPolicyIds: editGroup.value.allowedPolicyIds || '' })
    groupShow.value = false
    toast.success('已保存')
    loadAll()
  } catch (e: any) { toast.error(e.message) } finally { savingGroup.value = false }
}
function permSummary(g: any) {
  const p = g.appPerms || {}
  const vals = Object.values(p)
  const deny = vals.filter(v => v === false).length
  const allow = vals.filter(v => v === true).length
  if (!deny && !allow) return '全部默认'
  return `${allow} 允许 / ${deny} 禁止`
}
...
async function editPolicyRow(p: any) {
  editPolicy.value = {
    id: p.id, name: p.name, letter: p.letter, type: p.type, rootPath: p.rootPath,
    quotaGB: p.quotaBytes > 0 ? p.quotaBytes / (1 << 30) : 0
  }
...
async function loadUserShares() {
  try {
    allUserShares.value = (await adminApi.userShares(1, 200, userShareKw.value.trim())).items
  } catch (e: any) { toast.error(e.message) }
}
async function adminDelUserShare(s: any) {
  if (!(await uiDlg.confirm('取消共享', `取消共享「${s.name}」给 ${s.targetName}？对方将立即无法再访问该目录。`, { danger: true, okText: '取消共享' }))) return
  try { await adminApi.userShareDelete(s.id); loadUserShares() } catch (e: any) { toast.error(e.message) }
}
...
.sys-disk-head { display: flex; justify-content: space-between; font-size: 12px; margin-bottom: 4px; gap: 8px; }
/* 长挂载点路径（docker overlay 等）省略号截断，容量数字固定右侧不被挤出窗口 */
.sys-disk-name { font-weight: 600; flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.sys-disk-num { color: var(--text-3); font-variant-numeric: tabular-nums; flex: none; }
