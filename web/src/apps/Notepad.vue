<template>
  <div class="app-root">
    <div class="app-toolbar">
      <button class="tool-btn" @click="save" :disabled="!editable"><AppIcon name="check" :size="15" />保存</button>
      <div class="tool-sep"></div>
      <span style="font-size: 12px; color: var(--text-3)">{{ path || '未命名' }}</span>
      <div style="flex: 1"></div>
      <span style="font-size: 12px; color: var(--text-3)">{{ dirty ? '未保存' : '已保存' }} · {{ content.length }} 字符</span>
    </div>
