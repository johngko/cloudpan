...
- **图标体系修正：窗口最大化图标改 Windows 标准 + 应用撞图修复**：① 窗口控制「最大化」按钮此前误用四宫格（bi-app「田」字形）——Windows 惯例是**单个空心方框 □**，已改正；最大化后的「还原」为前后两叠层方框（此前即正确，微调对齐 Win11 线宽），最小化/关闭不变（关闭悬停红底白叉保持）。② **应用撞图修复**：图库与图片查看器共用 `image` 图标、媒体中心与媒体播放器共用 `media` 图标——为图库新设计**照片堆叠**图标（双照片叠放 + 山景，青绿→淡紫渐变）、为媒体中心新设计**场记板**图标（影片夹 + 两侧齿孔 + 播放三角，粉紫渐变），两两不再相同。③ 全桌面 26 个应用图标经脚本逐一两两比对（win12 主题 img/双主题 svg 双通道哈希）确认零重复；最小化「—」、关闭「×」、关闭悬停红底、还原/最大化切换（□ ⇄ 叠框）均按 Windows 逻辑验证。验证：GUI 桌面截图 + 图标哈希零重复 + 最大化/还原图标切换三态断言
  - **Icon system fixes: Windows-standard maximize glyph + app icon dedupe**: ① The window "maximize" button mistakenly used a four-square grid (bi-app, looks like 田); Windows convention is a **single hollow square □** — corrected; the post-maximize "restore" uses two overlapping squares (refined to Win11 stroke), minimize/close unchanged (close hover red bg with white cross kept). ② **Duplicate app icons fixed**: Photos (图库) shared the `image` icon with Image Viewer, and Media Center shared `media` with Media Player — Photos gets a new **photo-stack** icon (two overlapping photos + scenery, teal→lilac gradient) and Media Center a new **clapperboard** icon (film slate + side perforations + play triangle, pink-purple gradient). ③ All 26 desktop app icons verified pairwise unique via scripted hashing (win12 img / multi-theme svg dual-channel); minimize "—", close "×", close red hover, and maximize/restore toggling (□ ⇄ overlap) all verified per Windows logic. Verified with desktop screenshots + zero-duplicate icon hash + three-state maximize/restore assertions

- **内置 51 款免费商用字体（离线单文件）+ 离线清扫**：编辑器字体下拉此前依赖原平台接口（离线为空、无法切换字体），现改为**本地字体清单 `/fonts/index.json` + 全量字体文件随二进制内置**（嵌入模式自动走本地清单，不再请求外网）。新增 49 款：**中文 11 款**——思源黑体/思源宋体/站酷快乐体/站酷小薇LOGO体/站酷庆科黄油体/马善政毛笔楷书/柳建毛草/龙藏体/志莽行书（中文子集单文件 woff2 0.7-2.6MB，含思源黑体/宋体粗体）+ **西文/数字 38 款**——Inter/Roboto/Open Sans/Lato/Montserrat/Poppins/Oswald/Bebas Neue/Anton/Playfair Display/Merriweather/Nunito/Rubik/Work Sans/Dancing Script/Pacifico/Lobster/Caveat/Orbitron/Audiowide/Righteous/Archivo Black/Fira Code/JetBrains Mono/Roboto Mono/Source Code Pro/Space Mono/Courier Prime/Bungee/Fredoka/Comfortaa/Quicksand/Raleway/Ubuntu/Cinzel/Great Vibes/Indie Flower/Permanent Marker。**全部 OFL/免费商用授权**（fontsource 官方包），中文为简体子集单文件（现有 FontFace 单文件装载机制直接可用）。离线清扫：编辑器左栏外链 GitHub 图标（嵌入模式隐藏）、下载完成庆祝图改内联 SVG、二维码默认内容改本项目仓库。**全项目离线核查**：字体（本地清单+本地文件）、模板/素材（内置盘）、AI 抠图（本地 ONNX）、AI 配色（本地算法）、天气（外网但离线优雅降级）、缩略图（本地合成）——除天气与 AI 文案/生图两个明确降级项外，全部功能局域网离线可用。验证：GUI 字体下拉 51 款（中文分组含思源黑体/站酷系列/书法体，英文含 Inter/Roboto/Montserrat 等，各字体按自身字形预览）+ 应用字体渲染正常 + 回归 E2E 29+28+10+7 全绿、海报 GUI 11/11 全绿
  - **51 bundled free-for-commercial fonts (offline single-file) + offline sweep**: the editor's font dropdown previously depended on the original platform API (empty offline, no font switching); it now reads a **local font manifest `/fonts/index.json` with all font files shipped inside the binary** (embedded mode auto-uses the local manifest). 49 new fonts: **11 Chinese** — Noto Sans SC / Noto Serif SC / ZCOOL KuaiLe / ZCOOL XiaoWei / ZCOOL QingKe HuangYou / Ma Shan Zheng / Liu Jian Mao Cao / Long Cang / Zhi Mang Xing (single-file simplified-Chinese woff2 0.7-2.6MB, incl. bold weights) + **38 Latin/numeric** — Inter, Roboto, Open Sans, Lato, Montserrat, Poppins, Oswald, Bebas Neue, Anton, Playfair Display, Merriweather, Nunito, Rubik, Work Sans, Dancing Script, Pacifico, Lobster, Caveat, Orbitron, Audiowide, Righteous, Archivo Black, Fira Code, JetBrains Mono, Roboto Mono, Source Code Pro, Space Mono, Courier Prime, Bungee, Fredoka, Comfortaa, Quicksand, Raleway, Ubuntu, Cinzel, Great Vibes, Indie Flower, Permanent Marker. **All OFL/free-commercial** (official fontsource packages). Offline sweep: the side panel's external GitHub icon (hidden in embedded mode), the download-complete celebration image (inline SVG), and the QR-code default content (now this project's repo). **Project-wide offline check**: fonts (local manifest + files), templates/materials (built-in drive), AI cutout (local ONNX), AI palette (local algorithm), weather (online by design with graceful degradation), thumbnails (local compositor) — everything runs on a LAN without internet except weather and AI copy/image which degrade explicitly. Verified: GUI font dropdown with 51 fonts (Chinese group with Source Han/ZCOOL/calligraphy faces, Latin with Inter/Roboto/Montserrat etc., each previewed in its own typeface) + applied-font rendering + regressions E2E 29+28+10+7 green, poster GUI 11/11 green

- **海报设计万级资源库（模板 10,120 + 素材 11,670，全部程序自绘商用免费）+ AI 助手免登录本地配色 + 编辑器离线裁剪**：① **资源规模扩至万级**——新增程序化内容工厂（参数化扫描生成）：渐变背景 6,456、图案 3,600、边框/装饰 900+、参数化线形/面形图标 600+，叠加原 170 件手绘素材共 **11,670 件素材**；**10,080 张新模板**由 20 主题 × 12 版式族 × 42 变体组合生成（手机海报/横幅/方形三种尺寸，主题覆盖节日/促销/科技/自然/餐饮/美妆/婚礼/教育/招聘/健身/家居/音乐/旅行/电影/直播/电竞/书店/母婴/汽车/健康），叠加原 40 张精修模板共 **10,120 张模板**；缩略图由 PIL 合成器按模板 JSON 直接产出（260px，无需浏览器渲染）。**版权安全：所有素材/模板均为本项目代码参数化自绘，字体为站酷快乐体/站酷小薇（免费商用授权）与系统 Noto CJK（OFL），零第三方素材、可放心商用**。内置库版本 bump 至 20260913-1（合并式物化保留管理员自传内容）。② **AI 助手合理化**——去掉「请先登录后再使用 AI 功能」对云盘嵌入模式的不合理要求；**智能配色改为本地算法**（12 组主题关键词配色库 + 提示词哈希和谐五色，无登录无网络离线可用，实测「新年促销」→ 红金主题配色），文案/生图（依赖远端 AI 服务）在嵌入模式给出明确提示而非要求登录。③ **编辑器左侧面板离线裁剪**——嵌入模式下隐藏依赖原平台接口的 模板/素材/照片/我的 页签（避免「拼命中加载中」假死），保留离线可用的 文字/工具箱（含本地 AI 抠图）；系统模板/素材由应用左栏（系统库 + 我的库）提供。④ **画廊万级性能**——清单改经 raw 直链读取（绕过 readText 2MB 上限），模板/素材网格分块懒渲染（120/240 起步 + 触底自动加载 + 「加载更多」兜底）。验证：E2E 29+28 全绿（清单计数 10120/11670 断言）+ GUI 海报全链路 11/11（懒加载分块、分类筛选、编辑/保存/导出/多开）+ 海报资源管理 14/14 + AI 助手（无登录提示、本地配色 5 色生成、文案明确提示）
  - **Poster design: 10,120 templates + 11,670 materials (ten-thousand scale, all procedurally drawn & commercially safe) + login-free local AI palette + offline tab trimming + lazy gallery**: ① Resource scale expanded to ten-thousands via a procedural content factory (parameter sweeps): 6,456 gradient backgrounds, 3,600 patterns, 900+ borders/frames, 600+ parametric line/solid icons, plus the original 170 hand-drawn assets = **11,670 materials**; **10,080 new templates** generated from 20 themes × 12 layout families × 42 variants (phone/banner/square sizes; themes covering holidays, promo, tech, nature, food, beauty, wedding, education, hiring, fitness, home, music, travel, movie, live-commerce, esports, books, baby, auto, health) on top of the 40 hand-polished ones = **10,120 templates**; thumbnails are produced directly from template JSON by a PIL compositor (260px, no browser rendering). **Copyright safety: every template/material is drawn by this project's own parameterized code; fonts are ZCOOL KuaiLe/Xiaowei (free for commercial use) and system Noto CJK (OFL) — zero third-party assets**. Built-in version bumped to 20260913-1 (merge-materialization keeps admin uploads). ② AI assistant made reasonable — the "please log in first" demand no longer applies to embedded mode; **smart palette is now a local algorithm** (12 keyword-theme palettes + prompt-hash harmonized five-color sets, no login/network needed; "新年促销" → red-gold scheme), while AI copy/image (remote-service features) now show a clear notice instead of asking for login. ③ The editor side panel trims remote-dependent tabs (templates/materials/photos/mine, which hung at "loading") in embedded mode, keeping offline-capable text/tools (incl. local AI cutout); system templates/materials come from the app sidebar. ④ Gallery at ten-thousand scale — the manifest is fetched via raw link (bypassing the 2MB readText cap) and grids render lazily in chunks (120/240 initially + infinite scroll + load-more fallback). Verified: E2E 29+28 green (10120/11670 count assertions) + GUI poster chain 11/11, poster-asset management 14/14, AI assistant (no login prompt, 5-color local palette, clear copy notice)

- **终端安全边界（关键）+ 小组件新增「天气」与「SSH 快捷连接」**：① **本地终端收紧为管理员独占**——本地终端即项目运行服务器自身的 shell（服务以什么身份运行就是什么身份，本部署为 root），此前任何被授予「终端」功能的用户/访客都能经 `WS /terminal/ws?mode=local` 拿到服务器 shell；现后端对非 admin 的 local 模式直接 403（「本地终端仅管理员可用」），SSH 模式不受影响（各用户连自己登记的服务器，连接凭据本就按用户 AES-GCM 加密隔离）；前端同步：终端应用对非管理员隐藏「本地终端」页签并强制落在「远程 SSH」，资源管理器「在终端中打开」（本地终端语义）仅管理员可见。**安全链路三道闸：后端 WS 门禁（权威）→ 终端应用页签裁剪 → 资源管理器入口隐藏**。② 小组件面板新增 **天气** 卡（Open-Meteo 免费公共 API，无 key 只读 GET；CSP `connect-src` 精确放行该单一主机；8 个常用城市切换、选择持久化；离线/不可达时优雅降级「暂无法获取天气」）与 **SSH 快捷连接** 卡（列出当前用户保存的 SSH 连接，点击直接打开远程终端并连上该主机；仅对有终端权限的用户显示）；CSP 其余策略维持严格。验证：E2E 7/7（`/tmp/e2e-term.py`：授予 terminal 权限的普通用户/访客 local WS 均被 403、SSH 路径不受本地门禁影响、管理员不受限）+ GUI（管理员终端双页签、授权用户仅「远程 SSH」、资源管理器右键管理员有/普通用户无「在终端中打开」）+ 天气卡实时数据拉取成功（☀️ 29°C 晴）+ 回归 E2E 10+29 全绿、应用可见性 GUI 14/14 全绿
  - **Terminal security boundary (critical) + widgets: Weather & SSH quick-connect**: ① The LOCAL terminal is now admin-exclusive — it is a shell on the server running CloudPan itself (with whatever identity the service runs as; root in this deployment). Previously any user/guest granted the "terminal" feature could grab that shell via `WS /terminal/ws?mode=local`; the backend now rejects non-admin local mode with 403, while SSH mode is unaffected (each user connects only to their own registered servers, whose credentials are already per-user AES-GCM encrypted). Frontend mirrors it: the terminal app hides the "local" tab for non-admins and lands on "remote SSH", and Explorer's "open in terminal" (local semantics) is admin-only. Three layers: backend WS gate (authoritative) → terminal tab trimming → Explorer entry hidden. ② The widgets panel gains a **Weather** card (Open-Meteo free public API, keyless read-only GET; CSP `connect-src` allows exactly that one host; 8 preset cities with persisted selection; graceful "unavailable offline" degradation) and an **SSH quick-connect** card (lists the current user's saved SSH connections; click opens the remote terminal connected to that host; visible only to users with terminal permission). All other CSP directives remain strict. Verified: E2E 7/7 (granted user & guest local WS both 403, SSH path unaffected, admin unrestricted) + GUI (admin sees both tabs, granted user sees only remote SSH, Explorer context item admin-only) + live weather fetch (29°C sunny) + regressions E2E 10+29 green, app-visibility GUI 14/14 green

- **桌面显示全部应用 + 小组件实做（存储空间/传输任务/收藏）**：① **桌面图标改为显示全部可见应用**（此前仅带 desktop 标记的 14 个，其余只在开始菜单）——管理员桌面 26 个应用图标全量，普通用户仍严格按权限显示（组/个人权限与可安装应用安装态不变）；图标单元格加高至 102px 适配两行名称（如「文件资源管理器」「来自他人的共享」）不再与下一行重叠；三主题桌面共用此逻辑。② **任务栏小组件面板从「计算器 + 静态宣传卡」升级为实用网盘小组件**（保留计算器，右栏改为「网盘动态」实时卡片）：**存储空间**（各盘用量条形图 + 我的配额 已用/总量，不限量配额优雅显示）、**传输任务**（实时进度条 + 进行中徽标 + 状态文案，点击直达任务中心）、**收藏**（快速访问列表，点击直接打开所在目录的资源管理器）、CloudPan 项目卡。小组件数据全部本源（policies/transfer/stars），无外部依赖。验证：GUI 确认管理员桌面 26 图标全量、普通用户桌面仍按权限隐藏无权限项（应用可见性 14/14 全绿），小组件面板四卡渲染与实时数据正常
  - **Desktop shows every app + real widgets (storage / transfers / stars)**: ① Desktop icons now show all visible apps (previously only the 14 with a desktop flag; the rest were Start-menu-only) — the admin desktop shows all 26 app icons while normal users still see strictly permission-gated entries (group/user permissions and optional-app install state unchanged); icon cells grew to 102px so two-line names (e.g. 文件资源管理器) no longer overlap the next row; shared by all three themes. ② The taskbar widgets panel was upgraded from "calculator + static promo cards" to practical cloud-drive widgets (calculator kept; the right column is now a live "drive activity" feed): **Storage** (per-drive usage bars + my quota used/total, graceful unlimited display), **Transfers** (live progress bars, an in-progress badge, status text; click opens the Task Center), **Stars** (quick-access list; click opens Explorer at the starred folder), plus a CloudPan project card. All widget data is first-party (policies/transfer/stars) with zero external dependencies. Verified: GUI shows the admin desktop with all 26 icons while a normal user's desktop stays permission-filtered (app-visibility suite 14/14 green) and the widgets panel renders all four cards with live data

- **全面逻辑检查与前端 UI 审计（后端 + 真实 UI 操作模拟 + 全量截图），发现并修复 3 处隐藏性问题**：对桌面外壳、资源管理器、全部 21 个应用、管理控制台、设置、应用中心、三主题与深色模式做了系统性 GUI 审计（Playwright 驱动真实点击/键盘操作 + 每步截图 + 控制台错误收集），并核查 Windows 使用逻辑（单击选中/右键上下文菜单/新建后自动进入重命名/重名自动加序号/F2 重命名/Ctrl+C·V/Delete 入回收站/双击标题栏最大化/拖缘贴靠/任务栏点击恢复/开始菜单再点关闭）。**修复的问题**：① **思维导图从开始菜单启动（无文件）永久卡在加载态**——无文件路径未初始化模板数据，注入 null 导致编辑器初始化崩溃（destructure 'root' of null），现用模板初始化新导图、1 秒内可用；② **PDF 阅读器从开始菜单启动（无文件）白屏 + pdf.js 报 Invalid PDF**——新增友好空态「请在资源管理器中双击 PDF 文件打开」；③ **白板字体从 esm.sh CDN 加载（离线环境手写字体降级 + 每次 230 条 CSP 违规报错）**——字体资源本地化（public/excalidraw-assets/ 随二进制分发 + EXCALIDRAW_ASSET_PATH + 打包兜底 URL 指向本地），离线环境白板不再有外网依赖与报错。**核查确认无问题**：桌面外壳与窗口管理全部符合 Win11 习惯；资源管理器新建/重命名/复制/删除/搜索全链路正确；计算器运算正确（7×8=56）；管理控制台/设置/应用中心渲染完整；三主题切换与深色模式正常。回归：E2E 29+28+10 全绿，GUI 海报全链路 11/11、海报资源 14/14、应用可见性 14/14 全绿
  - **Full logic check & frontend UI audit (backend + real UI operation simulation + full screenshots), 3 hidden issues found and fixed**: systematic GUI audit over the desktop shell, File Explorer, all 21 apps, the admin console, Settings, App Center, all three themes and dark mode (Playwright driving real clicks/keyboard + per-step screenshots + console error collection), checking Windows usage conventions (click-to-select, context menus, auto-rename after create with auto-numbering, F2 rename, Ctrl+C/V, Delete-to-recycle-bin, double-click-titlebar maximize, edge snapping, taskbar click-to-restore, start-menu toggle). **Fixed**: ① Mind Map launched from the Start menu (no file) stuck on a loading spinner forever — the no-file path never initialized template data and injected null, crashing editor init ("destructure 'root' of null"); it now initializes a new map from the template and is usable within a second. ② PDF Reader launched with no file showed a blank window plus a pdf.js "Invalid PDF" error — replaced with a friendly empty state ("double-click a PDF in Explorer to open"). ③ Whiteboard fonts loaded from the esm.sh CDN (offline font degradation + 230 CSP violations per session) — font assets are now localized (public/excalidraw-assets/ shipped in the binary + EXCALIDRAW_ASSET_PATH + the bundled fallback URL pointed locally), so the whiteboard has no external dependency or errors offline. **Verified non-issues**: desktop shell and window management all match Win11 conventions; Explorer create/rename/copy/delete/search flows are correct; calculator math is right (7×8=56); admin console/Settings/App Center render fully; theme switching and dark mode work. Regression: E2E 29+28+10 all green, GUI poster chain 11/11, poster assets 14/14, app visibility 14/14 all green

- **管理员桌面显示全部应用，其他用户按权限显示**：此前系统功能被全局停用、组/个人无权限、或可安装应用未安装时，入口对所有用户（含管理员）都隐藏——管理员作为这些设置的设定者反而看不到自己「关掉」的应用。现改为：**管理员桌面/开始菜单/Dock/Launchpad/搜索 一律显示全部应用**（不受全局开关、组/个人权限、安装态影响），且可正常使用（后端 `AppAllowed` 对管理员先于全局开关放行，被停用功能的 API 对管理员不再 403）；其他用户与访客完全按管理员设置的权限显示（有权限显示、无权限隐藏，行为不变）。同步影响：资源管理器「打开方式」等按 `isAvailable` 过滤的入口对管理员同样全量放行。验证：E2E 10/10（`/tmp/e2e-apps-vis.py`：全局停用 speedtest 时管理员 ping 正常/普通用户 403、组权限禁用 poster 时管理员 allowed=true/普通用户 allowed=false）+ GUI 14/14（`/tmp/gui-test/gui-apps-vis.mjs`：管理员桌面出现 终端/媒体中心/图库/管理控制台、开始菜单全量；普通用户桌面/开始菜单按组权限只显示有权限的应用）
  - **Admin desktop shows all apps; other users strictly by permission**: previously a globally-disabled feature, missing group/user permission, or an un-installed optional app hid the entry for everyone — including the admin who configured it. Now the **admin's desktop/Start menu/Dock/Launchpad/search always show every app** (regardless of global toggles, per-group/user permissions, or install state) and they work (backend `AppAllowed` exempts admins before the global-enabled check, so APIs of disabled features no longer 403 for the admin); other users and guests are governed purely by admin-set permissions (visible if permitted, hidden if not — unchanged). The Explorer "open with" and similar `isAvailable`-filtered entries are likewise fully available to the admin. Verified: E2E 10/10 (`/tmp/e2e-apps-vis.py`: with speedtest globally disabled the admin pings OK while a normal user gets 403; with poster denied at group level the admin sees allowed=true while the user sees false) + GUI 14/14 (`/tmp/gui-test/gui-apps-vis.mjs`: admin desktop shows Terminal/Media Center/Photos/Admin Console and a full Start menu; a normal user's desktop/menu only shows permitted apps)

- **海报资源管理（管理员上传/用户隔离/授权）+ logo 更名「云盘设计」**（海报设计反馈三项）：① **管理员可随时上传/更换/删除内置模板与素材**——新增管理端接口 `POST/DELETE /api/admin/poster/builtin`（multipart：素材按分类入 `materials/<分类>/`、模板为 JSON+可选 png 缩略图入 `templates/`，同名覆盖=更换，单文件上限 20MB，路径防穿越校验），每次变更自动现扫 `templates/`+`materials/` 重建 `index.json` 清单（画布尺寸从模板 JSON 解析、缩略图同名 png 自动关联），编辑器左侧「系统模板/系统素材」即时可见并带上传/删除入口；物化语义改为**合并**（嵌入内容升级时覆盖同名、保留管理员上传的内容，index.json 恒由服务端现扫生成）。② **普通用户只可用、经授权才能管理自己的素材模板、用户间完全隔离**——模板库/素材库分「系统 | 我的」两个来源：「我的素材/我的模板」存各用户自己盘 `/poster/素材/`、`/poster/模板/`（天然按用户隔离），上传/删除需**用户组新权限「允许管理海报素材」**（`UserGroup.allowPosterAsset`，管理控制台用户组编辑勾选，`/auth/me` 透出，默认关闭；管理员豁免）；「存为模板」一键把当前画布连 JSON 带自动渲染的缩略图存进我的模板库（postMessage `cp-poster:export-all` 桥）；新增「我的作品」页签列出 `/poster/` 下全部作品点击继续编辑；管理员专属「用户资源」页签经 `GET /api/admin/poster/userlib|userfile` 查看任意用户的作品/素材/模板（普通用户不可见彼此、亦无此页签）。③ **编辑器 logo「迅排设计」更名「云盘设计」**（含页面标题与水印文案）。另修复素材分类筛选：清单 category 由目录名映射为中文（backgrounds→背景 等，历史目录为英文名且被模板 JSON 引用不可改名）。验证：E2E 29/29 + 28/28（`/tmp/e2e-poster.py`、`/tmp/e2e-poster2.py`：内置库上传/同名覆盖/删除/清单重建/穿越拒绝、非管理员 403、组授权标志透出、用户资源查看）+ GUI 11/11 + 14/14（`/tmp/gui-test/gui-poster.mjs`、`gui-poster2.mjs`：logo 改名、管理员上传素材 170→171→删回 170、存为模板→我的模板→重开、我的作品、用户资源查看、未授权无上传入口/授权后出现并可上传）
  - **Poster asset management (admin upload / user isolation / authorization) + logo renamed to 云盘设计** (three feedback items on Poster Design): ① Admins can upload/replace/delete built-in templates & materials at any time — new admin endpoints `POST/DELETE /api/admin/poster/builtin` (multipart: materials go to `materials/<category>/`, templates are JSON + optional png thumbnail into `templates/`; same-name upload replaces; 20MB per-file cap; path-traversal protected), and every change rescans `templates/`+`materials/` to rebuild the `index.json` manifest (canvas sizes parsed from template JSON, same-name png auto-linked as thumbnail) — the editor's "system templates/materials" panels show changes immediately and expose upload/delete controls for admins; materialization switched to **merge semantics** (an embedded-content upgrade overwrites same-name files and keeps admin-added ones; index.json is always generated server-side by scanning). ② Normal users can only use the feature; managing their own assets requires authorization and users are fully isolated — the template/material panels have "system | mine" sources: "my materials/templates" live in each user's own drive under `/poster/素材/` and `/poster/模板/` (naturally per-user isolated); uploading/deleting requires the new user-group permission **"allow poster asset management"** (`UserGroup.allowPosterAsset`, checkbox in the admin console group editor, exposed via `/auth/me`, off by default; admins exempt); "save as template" stores the current canvas (JSON + auto-rendered thumbnail) into my templates via the `cp-poster:export-all` postMessage bridge; a new "my works" tab lists everything under `/poster/` for one-click reopening; an admin-only "user resources" tab inspects any user's works/materials/templates via `GET /api/admin/poster/userlib|userfile` (users can never see each other's, and don't get the tab). ③ The editor logo 「迅排设计」 was renamed to 「云盘设计」 (page title and watermark copy included). Also fixed the material category filter: manifest categories are now mapped from directory names to Chinese (backgrounds→背景 etc., the historical directories are English-named and referenced by template JSONs so they cannot be renamed). Verified: E2E 29/29 + 28/28 (`/tmp/e2e-poster.py`, `/tmp/e2e-poster2.py`: builtin upload/same-name replace/delete/manifest rebuild/traversal rejection, non-admin 403, group flag exposure, user-library view) + GUI 11/11 + 14/14 (`/tmp/gui-test/gui-poster.mjs`, `gui-poster2.mjs`: renamed logo, admin upload 170→171→delete back to 170, save-as-template → my templates → reopen, my works, user-resource view, unauthorized users have no upload entry while authorized users do and can upload)

- **海报设计上线（集成 poster-design）+ 内置只读资源库 T:**：新增「海报设计」系统功能应用——把开源 poster-design（迅排设计，AGPL-3.0，仅前端集成）完整嵌入 `web/public/vendor/poster/`（约 64MB，含 AI 抠图本地模型，构建产物随单二进制分发、全离线可用），编辑器以同源 iframe cp 嵌入模式运行。配套内置**「海报资源库（T:）」只读内置盘**：`go:embed` 预置 40 张海报模板（节日/促销/餐饮/音乐/电影/教育/婚礼/科技/母婴/健身/美妆/家居/招聘/房产/旅行等 18 类题材，覆盖 750×1334 / 1920×1080 / 1000×1000 三种尺寸，每张带渲染缩略图）+ 170 个设计素材（背景 56 / 装饰 43 / 边框 22 / 图标 39 / 纹理 10）+ 2 款开源中文字体（站酷快乐体、站酷小薇体），启动时物化到 `data/builtin/poster` 并自动挂载进「此电脑」；T: 盘全链路只读——普通读写、管理员、分块上传、压缩、WebDAV、回收站、版本恢复、Office 回写全部拒绝并提示「内置资源库为只读」。使用流程：左侧模板库点选模板 → 编辑器改字换图（工具箱/文字/素材面板，AI 抠图为本地 u2netp 推理）→ 素材库单击素材直接加到画布 → 「保存」把作品写成 `.poster.json` 落到本机盘 `/poster/`（资源管理器双击可重开继续编辑、原位覆盖保存）→「导出 PNG 到云盘」（前端 html2canvas 渲染 + 分块上传到 `/poster/`）或「下载 PNG」到本地；支持按文档多开。技术要点：CSP 仅对 `/vendor/poster/` 子树放宽 `script-src 'unsafe-eval'`（vue-i18n 运行时消息编译依赖 `new Function`，其余路径维持严格 CSP）；模板内素材引用自动改写为云盘直链；应用窗口尺寸回退应用注册表默认值；走系统功能门控（管理控制台可全局启停 + 按用户组分配）。验证：E2E 29/29（manifest key、T: list/text/raw、四类写拒绝、保存回读、vendor 页 CSP/XFO、AppGate 启停）+ GUI 全链路 11/11（开应用、模板库 40/素材库 170、分类筛选、模板渲染、素材上画布、保存落盘、PNG 导出上传、资源管理器双击重开、多开）
  - **Poster Design (poster-design integration) + built-in read-only asset library T:** New "Poster Design" system app — the open-source poster-design editor (AGPL-3.0, frontend-only integration) is vendored whole into `web/public/vendor/poster/` (~64MB incl. the local AI-cutout model, shipped inside the single binary, fully offline) and runs in a same-origin iframe under a cp embed mode. It ships with a built-in **read-only "Poster Assets (T:)" drive**: `go:embed` preloads 40 poster templates (18 themes — holidays, promo, food, music, movie, education, wedding, tech, baby, fitness, beauty, home, hiring, real estate, travel…; in 750×1334 / 1920×1080 / 1000×1000 sizes, each with a rendered thumbnail) + 170 design assets (56 backgrounds / 43 decorations / 22 borders / 39 icons / 10 textures) + 2 open-source Chinese fonts (ZCOOL KuaiLe, ZCOOL XiaoWei), materialized at startup into `data/builtin/poster` and auto-mounted into "This PC". T: is enforced read-only across the whole stack — normal writes, admin, chunked upload, archive, WebDAV, recycle bin, version restore and Office save-back are all rejected with a "built-in library is read-only" message. Workflow: pick a template in the left gallery → edit text/images in the editor (text/tools/material panels; AI cutout runs locally via u2netp) → click a material to drop it onto the canvas → 保存 saves the work as `.poster.json` to your own drive under `/poster/` (double-click in Explorer to reopen and keep editing, saving overwrites in place) → "Export PNG to cloud" (frontend html2canvas render + chunked upload to `/poster/`) or "Download PNG" locally; documents open in multiple windows. Technical notes: CSP relaxes `script-src 'unsafe-eval'` only for the `/vendor/poster/` subtree (vue-i18n's runtime message compiler needs `new Function`; all other paths keep the strict CSP); in-template asset references are rewritten to cloud-drive direct links; app windows fall back to registry default sizes; gated as a system feature (admin can toggle globally and assign per group). Verified: E2E 29/29 (manifest key, T: list/text/raw, four write-rejection paths, save round-trip, vendor-page CSP/XFO, AppGate toggle) + GUI full-chain 11/11 (open app, gallery 40/materials 170, category filter, template render, material onto canvas, save to drive, PNG export upload, Explorer double-click reopen, multi-open)

- **win12 任务栏通知中心修复**（2 处反馈 + 全面排查发现的 2 处潜在逻辑问题）：① **点铃铛无反应**——根因是点击处理器把本地面板状态设为 `'notify'`，但模板里根本没有该状态的渲染分支，真正驱动通知面板的 store `open` 状态从未被置位（macos/deepin 主题铃铛一直是对的，仅 win12 有错）；现点铃铛直接开/关通知中心（Win11 同款），面板打开期间铃铛按钮保持高亮。② 「有未读角标但看不到通知」的根因是只能从控制中心里的「通知中心」磁贴进入（该路径本身正常，实测 35 条全部显示）——铃铛修好后入口一步直达。③ 顺带修复通知 store 两处潜在逻辑错误：**未读数曾按前 50 条列表重算**（列表上限 50 条，未读超过 50 时角标会被算小，如 120 条只显示 50）——现改为列表与专用计数接口并行取数；**点击已读条目仍会减未读数**——现仅在真正未读→已读时减一。验证：GUI 9/9（角标计数、点铃铛开面板+条目显示、再点关闭、控制中心与磁贴路径回归、点未读条目角标减一、重复点已读条目不变）
  - **Win12 taskbar notification-center fixes** (two reported issues + two latent logic issues found during a full review): ① The system-tray bell did nothing when clicked — the handler set a local panel state to `'notify'` for which the template had no rendering branch, and the store `open` flag that actually drives the notification panel was never set (the macos/deepin theme bells were correct all along; win12 was the only broken one); the bell now opens/closes the notification center directly (Win11-style) and the tray button stays highlighted while the panel is open. ② "Badge shows unread but no notifications visible" turned out to be because the only way in was the "Notification Center" tile inside the control center (that path itself worked — all 35 items rendered in testing); with the bell fixed, the list is now one click away. ③ Two latent notify-store logic issues fixed along the way: the unread count was recomputed from the capped (50-item) list, which under-counts the badge when unread > 50 (e.g. 120 shown as 50) — it now fetches the dedicated count endpoint in parallel with the list; and clicking an already-read item still decremented the unread count — it now decrements only on a real unread→read transition. Verified: GUI 9/9 (badge count, bell click opens panel with items, second click closes, control-center + tile path regression, unread-item click decrements badge by one, repeat click on a read item is a no-op)
